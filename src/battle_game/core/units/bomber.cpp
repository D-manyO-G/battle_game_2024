...
Bomber::Bomber(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void Bomber::Update() {
  BomberMove(3.0f, glm::radians(180.0f));
  TurretRotate();
  Fire();
}

void Bomber::BomberMove(float move_speed, float rotate_angular_speed) {
...
bool Bomber::IsHit(glm::vec2 position) const {
...
const char *Bomber::UnitName() const {
  return "Normal Bomber";
}

const char *Bomber::Author() const {
  return "Dooogs";
}
}  // namespace battle_game::unit
